...
const solution = const solution = input.billing.paymentMethod
...
const expected = input.orders.map(x => ({productId: x.productId + '-' +x.storeLocale}))
...
const result = input.orders.filter(order => order.price > 10).map(order => order.productId).toString
...
const result = input.orders.reduce((acc, current) => acc = acc + current.price, 0)
